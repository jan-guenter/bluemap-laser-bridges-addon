...
"""Generate the bounded deterministic Laser Bridges gallery."""
...
def placements_tsv() -> bytes:
    lines = [
        "case_id\tlabel\tx\ty\tz\tblock_state\tverify_state\tplace\texpected"
    ]
    lines.extend(
        "\t".join(
            (
...
def verify_function() -> bytes:
    ...
    for placement in cases.PLACEMENTS:
        verified_state = placement.verify_state or placement.block_state
        lines.append(
            f"execute unless block {placement.x} {placement.y} {placement.z} "
            f"{verified_state} run tellraw @a "
            f"{{\"text\":\"gallery mismatch: {placement.case_id}\",\"color\":\"red\"}}"
        )
    lines.append(
        "tellraw @a {\"text\":\"Laser Bridges & Doors gallery checked\","
        "\"color\":\"green\"}"
    )
    return text_bytes("\n".join(lines))
...
def base_files() -> dict[Path, bytes]:
    ...
    return {
        ...
        Path("datapack/pack.mcmeta"): json_bytes(
            {
                "pack": {
                    "description": "ATM 1.2.0 Laser Bridges & Doors tint gallery",
                    "pack_format": 48,
                }
            }
        ),
        ...
        function_root / "load.mcfunction": text_bytes(
            "# Generated by gallery/generate.py; do not edit.\n"
            "tellraw @a {\"text\":\"Laser Bridges & Doors tint gallery loaded; "
